// app/v2/controller/createFile.js
...
class CreateFileController extends Controller {
  ...
  async createTestcaseFile(id) {
    ...
    switch (result.programLang) {
      case 'js':
        await this.createJsTestCaseFile(result);
        // this.createJsRunFile()
        break;
      case 'java':
      case 'c':
      case 'cpp':
      default: ctx.throw(406, '暂不支持' + result.programLang + '编程语言的测试用例文档生成');
    }
  }

  // 在以 codingQeustions 的 id为名的文件夹中创建符合Js语法可被Js直接调用的测试用例文档 testcase.js
  async createJsTestCaseFile(testcase) {
    const ctx = this.ctx;
    const dataFileUrl = new URL(reservePath + testcase.cqId + '/testcases.js');
    // const runFileUrl = new URL(reservePath + testcase.cqId + '/run.js');
    const dirUrl = new URL(reservePath + testcase.cqId);
    ...
    // 处理 JSON 字符串
    const inputArray = JSON.parse(testcase.inputData);
    const outputArray = JSON.parse(testcase.outputData);
  ...
  async createJudgeFile(params) {
    ...
    switch (quest.programLang) {
      case 'js':
        // 参数依次是  ‘需要测试的用户代码’，‘预设的函数名称’，‘适配本题的测试程序代码’，‘编程语言’，‘题目Id’，‘用户Id’
        await this.createJsJudgeFile(codingRecord.code, quest.preFuncName, testProgram.testCode, quest.programLang, params.cqId, params.uId);
        break;
      case 'java':
      case 'c':
      case 'cpp':
      default: ctx.throw(406, '暂不支持' + quest.programLang + '编程语言的测试用例文档生成');
    }
  }

  async createJsJudgeFile(code, preFuncName, testCode, programLang, cqId, uId) {
    const ctx = this.ctx;
    // 拼接出完整的，可以带入testcases并运行的判题文档
    let content = code +
                  'let solution = ' + preFuncName + '\n' +
                  'const testCases = require("../' + cqId + '/testcases.js")\n' +
                   testCode;
    ...
    const judgeFileUrl = new URL(reservePath + 'submitCode/' + cqId + '-' + uId + '.' + programLang);
